...
export default function DepartmentSidebar({ mobileOpen, setMobileOpen }) {
  const navigate = useNavigate();

  const navItems = [
    { name: 'Dashboard', path: '/department/dashboard', icon: LayoutDashboard },
    { name: 'Assigned Complaints', path: '/department/complaints', icon: ClipboardList, count: 42 },
    { name: 'In Progress', path: '/department/in-progress', icon: LoaderCircle, count: 12 },
    { name: 'Resolved', path: '/department/resolved', icon: CheckCircle2, count: 22 },
    { name: 'Notifications', path: '/department/notifications', icon: Bell, count: 4 },
    { name: 'Department Analytics', path: '/department/analytics', icon: BarChart3 },
  ...
  return (
    <>
      ...
      <aside
        ...
        aria-label="Department Staff Navigation"
      >
        {/* Brand Header */}
        <div className="p-5 flex items-center justify-between border-b border-[#1A2E3B] bg-[#050A0C]/60 relative">
          <div className="w-full">
            <UniversityLogo variant="sidebar" subtitle="Department Portal" linkTo="/department/dashboard" />
          </div>
                    ...
                    <div className="flex items-center gap-3">
                      <Icon
                        className={`w-4 h-4 transition-colors ${
                          isActive ? 'text-[#D4A84F]' : 'text-[#9FB1BC]'
                        }`}
                      />
                      <span>{item.name}</span>
                    </div>
        ...
        {/* Bottom User/Support Controls */}
        <div className="p-3 border-t border-[#1A2E3B] bg-[#050A0C]/50 space-y-1">
          <button
            onClick={() => alert('Department Helpdesk: Contact extension 4022 or maint-ops@campus.edu')}
