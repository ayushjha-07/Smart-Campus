    ...
    <header className="sticky top-0 z-30 h-16 bg-[#07121A]/95 backdrop-blur-md border-b border-[#1A2E3B] px-4 lg:px-8 flex items-center justify-between transition-colors">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-base sm:text-lg font-bold text-[#F5F5F0] tracking-tight flex items-center gap-2">
            {title}
            <span className="hidden md:inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-[#315C3A]/25 text-[#D4A84F] border border-[#315C3A]/60">
              <Wrench className="w-3 h-3 text-[#D4A84F]" />
              Staff Console
            </span>
          </h1>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <form onSubmit={handleSearchSubmit} className="relative hidden md:block">
          ...
          <input
            type="text"
            placeholder="Search assigned tickets, IDs..."
            value={query}
            onChange={(e) => {
            ...
            className={`w-44 lg:w-60 pl-9 pr-8 py-1.5 text-xs bg-[#0D1B22] border rounded-lg text-[#F5F5F0] placeholder-[#9FB1BC]/60 focus:outline-none transition-all ${
              searchFocused
                ? 'border-[#D4A84F] ring-1 ring-[#D4A84F]/30 w-68'
            ...
            <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-[#0D1B22] border border-[#1A2E3B] rounded-xl shadow-2xl p-3 z-50 text-xs animate-in fade-in slide-in-from-top-2 duration-150">
              <div className="flex items-center justify-between pb-2 border-b border-[#1A2E3B]">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-[#F5F5F0]">Department Alerts</span>
                  {unreadCount > 0 && (
              ...
              <div className="divide-y divide-[#1A2E3B] max-h-64 overflow-y-auto my-1 scrollbar-thin">
                {notifications.length === 0 ? (
                  <div className="py-6 text-center text-[#9FB1BC]">
                    No department alerts available
                  </div>
                ) : (
                  notifications.slice(0, 5).map((n) => (
                    <div
                      ...
                      className={`py-2.5 px-2 hover:bg-[#13242E] rounded transition-colors cursor-pointer ${
                        !n.is_read ? 'bg-[#07121A]/50 border-l-2 border-l-[#D4A84F]' : ''
                      }`}
                    >
                      ...
                      <p className="text-[11px] text-[#A8B3B0] mt-0.5 line-clamp-2">{n.message}</p>
                    </div>
                  ))
                )}
              </div>
        ...
        <div className="flex items-center gap-2 pl-1">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#315C3A] to-[#13242E] border border-[#D4A84F]/50 flex items-center justify-center text-[#D4A84F] text-xs font-bold shadow-sm">
            {DEPARTMENT_STAFF_PROFILE.avatarInitials}
          </div>
          <div className="hidden sm:flex flex-col text-left">
            <span className="text-xs font-semibold text-[#F5F5F0] leading-none">
              {DEPARTMENT_STAFF_PROFILE.name}
            </span>
            <span className="text-[10px] text-[#71844A] font-medium leading-tight mt-0.5">
              {DEPARTMENT_STAFF_PROFILE.role} • {DEPARTMENT_STAFF_PROFILE.employeeId}
            </span>
          </div>
