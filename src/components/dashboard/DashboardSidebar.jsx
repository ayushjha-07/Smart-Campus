...
export default function DashboardSidebar({ mobileOpen, onCloseMobile }) {
  ...
  const sidebarBody = (
    <div className="relative flex flex-col h-full justify-between select-none overflow-hidden text-white">
      ...
      {/* Subtle top and bottom readability gradients (middle gate arch is completely open and bright) */}
      <div 
        className="absolute inset-x-0 top-0 h-72 z-0 pointer-events-none"
        style={{
          background: 'linear-gradient(180deg, rgba(5, 13, 18, 0.55) 0%, rgba(5, 13, 18, 0.20) 45%, transparent 100%)'
        }}
      />
      <div 
        className="absolute inset-x-0 bottom-0 h-64 z-0 pointer-events-none"
        style={{
          background: 'linear-gradient(0deg, rgba(5, 13, 18, 0.75) 0%, rgba(5, 13, 18, 0.30) 60%, transparent 100%)'
        }}
      />
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 group ${
                  isActive
                    ? 'bg-[#078A5A] text-white shadow-lg font-bold backdrop-blur-xs'
                    : 'text-white hover:bg-black/25 hover:text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.85)]'
                }`}
              >
      ...
      <div className="relative z-10 p-4 border-t border-white/15 space-y-2.5">
        <div className="px-2">
