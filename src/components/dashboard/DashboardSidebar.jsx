            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-all duration-200 group ${
                  isActive
                    ? 'bg-[#079669] text-white shadow-md shadow-emerald-950/40 font-bold'
                    : 'text-white/85 hover:text-white hover:bg-white/[0.08] drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]'
                }`}
              >
                <div className="flex items-center gap-3">
                  <Icon className={`w-4 h-4 transition-colors ${isActive ? 'text-white' : 'text-white/80 group-hover:text-white'}`} />
                  <span>{item.name}</span>
                </div>
                {item.name === 'Help & Support' && (
                  <ChevronRight className={`w-3.5 h-3.5 transition-colors ${isActive ? 'text-white' : 'text-white/50 group-hover:text-white'}`} />
                )}
              </Link>
            );
          })}
