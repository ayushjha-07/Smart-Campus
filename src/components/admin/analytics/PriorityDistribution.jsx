    ...
    return (
      <div className="bg-[#050A0C]/95 border border-[#1A2E3B] rounded-lg p-2.5 shadow-xl text-xs backdrop-blur-md">
        <div className="flex items-center gap-2 mb-1">
          ...
          />
          <span className="font-semibold text-[#F5F5F0]">{data.name} Priority</span>
        </div>
...
export default function PriorityDistribution({ data }) {
  ...
  return (
    <div className="bg-[#0D1B22] border border-[#1A2E3B] rounded-xl p-5 shadow-lg flex flex-col justify-between h-full">
      ...
      <div className="flex items-center justify-between pb-3 mb-2 border-b border-[#1A2E3B]/80">
        <div>
          ...
          <p className="text-xs text-[#9FB1BC] mt-0.5">
            Severity segmentation of campus complaints
          </p>
        </div>
      ...
      {/* Legend & Breakdown */}
      <div className="grid grid-cols-2 gap-2 pt-3 border-t border-[#1A2E3B]/60 text-xs">
        {data.map((item) => (
          <div
            key={item.name}
          ...
          >
            ...
            <div className="text-right shrink-0">
              ...
              <span className="text-[10px] text-[#71844A]">({item.percentage}%)</span>
            </div>
          </div>
        ))}
      </div>
