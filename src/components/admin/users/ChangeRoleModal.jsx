...
import { ROLES } from '../../../data/userManagementMockData';

export default function ChangeRoleModal({ isOpen, onClose, user, onUpdateRole }) {
      ...
      <div className="bg-[#07121A] border border-[#1A2E3B] rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col text-xs">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1A2E3B] bg-[#0D1B22]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-[#F5F5F0]">Change User Role</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="p-3 bg-[#0D1B22] border border-[#1A2E3B] rounded-xl flex items-center justify-between">
            <span className="text-[#9FB1BC]">Current Assigned Role:</span>
            <span className="font-bold text-[#D4A84F]">{user.role}</span>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 flex items-start gap-2.5">
            ...
            <p className="text-[11px] leading-relaxed">
              <strong>Warning:</strong> Changing a user's role can change their access permissions. Department staff and administrator privileges allow editing tickets and institutional routing.
            </p>
          </div>
