...
export default function EditDepartmentModal({ isOpen, onClose, dept, onSaveDepartment }) {
  const [name, setName] = useState(dept?.name || '');
  const [code, setCode] = useState(dept?.code || dept?.id || '');
  const [head, setHead] = useState(dept?.head || '');
  const [description, setDescription] = useState(dept?.description || '');
  const [status, setStatus] = useState(dept?.status || 'Active');
  const [selectedCategories, setSelectedCategories] = useState(dept?.categories || []);
      ...
      <div className="bg-[#07121A] border border-[#1A2E3B] rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#1A2E3B] bg-[#0D1B22]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-[#F5F5F0]">Edit Department Details</h2>
              <p className="text-[11px] text-[#9FB1BC]">{dept.code || dept.id} • {dept.name}</p>
            </div>
          </div>
          <button
            onClick={onClose}
