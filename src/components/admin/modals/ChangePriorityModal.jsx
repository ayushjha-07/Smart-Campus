...
import { PRIORITY_OPTIONS } from '../../../data/adminMockData';
...
const priorityConfig = {
  ...
  HIGH: {
    ...
    border: '#F97316',
    desc: 'Major disruption, rapid dispatch required within 12 hours',
  },
  CRITICAL: {
    color: '#EF4444',
    bg: 'rgba(239, 68, 68, 0.15)',
    border: '#EF4444',
    desc: 'Safety or campus infrastructure hazard, immediate response',
  },
};

export default function ChangePriorityModal({ isOpen, onClose, complaint, onUpdatePriority }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onUpdatePriority(complaint.id, selectedPriority);
    onClose();
  };
      ...
      <div className="relative w-full max-w-md bg-[#0D1B22] border border-[#1A2E3B] rounded-xl shadow-2xl p-6 overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-[#1A2E3B]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-[#F5F5F0]">Change Priority</h3>
              <p className="text-xs text-[#9FB1BC]">Update ticket triage level</p>
            </div>
          </div>
          <button
            onClick={onClose}
