...
import { COMPLAINT_DEPARTMENTS } from '../../../../data/adminComplaintsData';
...
export default function AssignDepartmentModal({
  ...
  const [selectedDept, setSelectedDept] = useState(
    complaint?.department || COMPLAINT_DEPARTMENTS[1] // Maintenance
  );
      ...
      <div className="relative w-full max-w-md bg-[#0D1B22] border border-[#1A2E3B] rounded-2xl shadow-2xl p-6 overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-[#1A2E3B]">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-[#315C3A]/20 border border-[#315C3A]/40 text-[#D4A84F]">
            ...
            <div>
              ...
              <p className="text-xs text-[#9FB1BC]">
                Route ticket to specialized campus operational unit
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 text-xs">
          ...
          {isBulk && (
            <div className="p-3 rounded-lg bg-[#07121A] border border-[#1A2E3B] text-xs text-[#9FB1BC]">
              You are reassigning <strong className="text-[#F5F5F0]">{selectedCount}</strong> selected complaints simultaneously.
            </div>
          )}
          ...
          {!isBulk && complaint && selectedDept !== complaint.department && (
          ...
          <div>
            <label className="block font-medium text-[#9FB1BC] mb-1.5">
              Assignment Note <span className="text-[10px] text-[#9FB1BC]/60">(Optional)</span>
            </label>
            <textarea
              rows={2}
              value={note}
              onChange={(e) => setNote(e.target.value)}
              placeholder="Add specific instructions for the receiving department lead..."
